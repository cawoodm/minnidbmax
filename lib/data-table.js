...
export class DataEntryTable extends HTMLElement {
  ...
  establishColumns(values) {
    ...
    this.filters = new Array(this.columnTypes.length).fill("");
  }
  ...
  renderTable(displayData) {
    ...
    this.columnNames.forEach((name, index) => {
      const dataType = this.columnTypes[index];
    ...
    tableHTML +=
      ...
      this.columnTypes
        .map((type, index) => {
          return `<td><input class="filter-input" fieldIndex="${index}" value="${this.filters[index]}"/></td>`;
        })
        .join(" ") +
    ...
    displayData.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, cellIndex) => {
        ...
        const dataType = this.columnTypes[cellIndex];
        let classNames = [dataType];
